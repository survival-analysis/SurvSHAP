...
class PermutationExplainer(Loggable):
    def __init__(self, results_folder, prefix_name, n_iters=10):
    ...
    def fit(self, model, x, y, features_names_list):
        self.model = model
        self.x = x
        self.y = y
        self.features_names_list = features_names_list
        assert x.shape[1] == len(features_names_list), 'X columns and features_names_list should have the same length or features_names_list will be considered the first n features in X'

    def feature_importance(self):
        perm_imp_dict = {'Feature': [], 'Value': []}
        for f in range(self.x.shape[1]):
            bsc = self.model.score(self.x, self.y) * 100
            xp = self.x.copy()
            diffcs = []
            for i in range(self.n_iters):
                np.random.shuffle(xp[:, f])
                asc = self.model.score(xp, self.y) * 100
                diffc = bsc - asc
                diffcs.append(diffc)
            diffc_avg = np.mean(diffcs)
            perm_imp_dict['Feature'].append(self.features_names_list[f])
            perm_imp_dict['Value'].append(diffc_avg)
            # print('Diff Score: {:.2f}'.format(diffc_avg))
            # print('-------------------------------')
        return pd.DataFrame(perm_imp_dict)


class XSurv(Loggable):
    def __init__(self, prefix_name, results_folder='Results', max_k=30, z_explained_variance_ratio_threshold=99.0, curves_diff_significance_level=0.05, verbose=True):
    ...
    def _get_z(self):
        ...
        for i in range(1, self.survival_curves_train.shape[1]+1):
            ...
            if pca.explained_variance_ratio_.sum() >= self.z_explained_variance_ratio_threshold:
    ...
    def _cluster_in_z(self, pretrained_model=None):
        if pretrained_model is None:
            # self.clustering_mdl = KMeans(n_clusters=self.optimal_k, n_init=10).fit(self.z_train)
    ...
    def _find_optimal_clusters_number(self):
        ...
        for k in range(2, self.max_k):
            diffs_temp = []
            for i in range(1):
                d = self._count_logrank_diffs(self.z_train, ye=self.ey_train_surv, e=self.e_train, number_clusters=k)
                diffs_temp.append(d)
        ...
        # last_max_diff_idx = len(diffs) - diffs[::-1].index(max(diffs)) + 1
        ks = list(range(2, self.max_k))

        if self.verbose:
            plt.figure(figsize=(4, 4))
            plt.plot(ks, diffs)
            plt.fill_between(ks, diffs - diffs_stds, diffs + diffs_stds, alpha=0.2)
    ...
    def _count_logrank_diffs(self, data, ye, e, number_clusters):
        mdl = self._base_clustering_model(number_clusters=number_clusters).fit(data)
...
class XSurvKMeansRF(XSurv, ShapExplainer):
    def __init__(self, max_depth=10, *args, **kwargs):
        self.max_depth = max_depth
        super().__init__(*args, **kwargs)

    def _base_clustering_model(self, number_clusters):
        return KMeans(n_clusters=number_clusters, n_init=10, random_state=10)
...
class XSurvGMMRF(XSurv, ShapExplainer):
    def _base_clustering_model(self, number_clusters):
        return GaussianMixture(n_components=number_clusters, n_init=10)

    def _classify(self):
        rf = RandomForestClassifier(max_depth=22, random_state=0, oob_score=True, class_weight='balanced_subsample')
        rf.fit(self.x_train, self.labels_train)
        return rf
